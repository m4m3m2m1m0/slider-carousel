if (navigator.msMaxTouchPoints) {
  ...
  var slider = {
    ...
    bindUIEvents: function () {
      ...
      this.el.holder.on("mousedown", function (event) {
        this.mouseDown = true;
        slider.start(event);
      });

      this.el.holder.on("mousemove", function (event) {
        if (this.mouseDown) {
          slider.move(event);
        }
      });

      this.el.holder.on("mouseup", function (event) {
    ...
    move: function (event) {
      ...
      var navButtons = this.el.nav[0].children;
      $(navButtons).removeClass();
      $(navButtons[this.index]).addClass('active-nav');
    }
  };
